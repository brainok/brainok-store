...
const defaultSiteSettings = {
    ...
    heroDescription: "Publish apps with thumbnails, demos, installers, and universal Brainok licenses in one place. Every app can start with a 30-day trial.",
...
function emailPreviewText(value, maxLength = 6000) {
...
function inlineMarkdownToHtml(value) {
    ...
    return escaped.replace(/\[([^\]]+)\]\((https?:\/\/[^)\s]+)\)/g, (_match, label, url) => `<a href="${htmlAttribute(url)}">${label}</a>`);
}
...
function readmeImageLinksHtml(images, label) {
    if (images.length === 0) {
        return "";
    }
    return [
...
function secretOrEnv(secret, envName) {
    try {
...
function normalizePublicLicensePlan(value) {
...
function licenseRequestEmailContent({ requestId, name, email, plan, requestedPlanLabel, devices, message }) {
...
function appAnnouncementEmailContent({ appName, appType, category, latestVersion, shortDescriptionKo, shortDescriptionEn, readmeKo, readmeEn, appUrl }) {
...
async function sendResendEmail({ to, subject, text, html, licenseCode, licenseId, requestId, replyToEmail, type, requestedByUid }) {
    ...
    if (!apiKey) {
        ...
        throw new HttpsError("failed-precondition", "RESEND_API_KEY is not configured. Set it with Firebase Secret Manager or a local environment variable.");
    }
    ...
    if (result.error) {
        ...
        throw new HttpsError("internal", result.error.message || "Resend could not send the license email.");
    }
...
async function sendResendTestLicenseEmail(requestedByUid) {
...
async function requireSiteAdmin(uid) {
...
function paymentStatus(value) {
    return oneOf(value, ["created", "pending", "paid", "paid_needs_license", "refunded", "partially_refunded", "failed", "cancelled"], "pending");
}
async function writePaymentRecord({ provider, status, email, amountCents, currency, orderId, providerPaymentId, rawPayload, licenseId, licenseCode }) {
    ...
    await paymentRef.set(compactMap({
...
export const tossPaymentsWebhook = onRequest({ region, cors: false }, async (request, response) => {
    ...
    const eventName = asString(request.get("toss-webhook-event-type")) ||
...
export const createCheckout = onCall({ region }, async (request) => {
    ...
    if (provider.id === "toss") {
        throw new HttpsError("failed-precondition", "Toss Payments checkout is not configured yet. Manual license issuance is available.");
    }
...
function licenseIdForCode(code) {
...
function normalizePlan(value) {
    ...
    return oneOf(raw, ["personal", "pro", "lab", "friend"], "personal");
}
...
function appSettingsFromRequest(data, defaults = {}) {
    const pricingMode = oneOf(data.pricingMode, ["invite_only", "free", "paid"], defaults.pricingMode || "invite_only");
...
export const updateApp = onCall({ region }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const settings = appSettingsFromRequest(data, {
            pricingMode: oneOf(existingPricing.mode, ["invite_only", "free", "paid"], "invite_only"),
...
async function activeLicenseRecipientEmails(maxRecipients) {
...
export const sendAppAnnouncement = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const uid = requireUid(request);
    ...
    const appType = oneOf(app.appType, ["application", "web_app"], "application");
    ...
    await announcementRef.set(compactMap({
...
export const requestBrainokLicense = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const data = (request.data || {});
    ...
    const language = oneOf(data.language, ["ko", "en"], "en");
...
export const createLicense = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const uid = requireUid(request);
    await requireSiteAdmin(uid);
    const data = (request.data || {});
...
function licenseEmailAddress(data, license) {
...
async function sendStoredLicenseEmail({ requestData, requestedByUid, type }) {
    ...
    const status = oneOf(license.status, ["active", "disabled", "expired"], "active");
    if (status !== "active") {
...
export const verifyLicense = onCall({ region }, async (request) => {
    ...
    const allowedApps = Array.isArray(license.allowedApps)
        ? license.allowedApps.filter((value) => typeof value === "string")
        : ["*"];
    const appAllowed = !appId || allowedApps.includes("*") || allowedApps.includes(appId);
    let status = oneOf(license.status, ["active", "disabled", "expired"], "active");
...
export const sendLicenseEmail = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const uid = requireUid(request);
    await requireSiteAdmin(uid);
...
export const resendLicenseEmail = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const uid = requireUid(request);
    await requireSiteAdmin(uid);
    return sendStoredLicenseEmail({
        requestData: (request.data || {}),
...
export const sendTestLicenseEmail = onCall({ region, secrets: [resendApiKey] }, async (request) => {
    const uid = requireUid(request);
    await requireSiteAdmin(uid);
...
export const activateBrainokLicense = onCall({ region }, async (request) => {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        const [activationSnap, activeActivationsSnap] = await Promise.all([
            transaction.get(activationRef),
            transaction.get(db.collection("activations")
                .where("licenseId", "==", licenseId)
                .where("status", "==", "active"))
        ]);
        const status = oneOf(license.status, ["active", "disabled", "expired"], "active");
...
export const createAppCheckout = onCall({ region }, async (request) => {
    ...
    const mode = oneOf(pricing.mode, ["invite_only", "free", "paid"], "invite_only");
    if (mode !== "paid") {
